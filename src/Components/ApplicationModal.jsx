...
export default function ApplicationModal({ isOpen, onClose, job }) {
  ...
  const daysLeft = Math.max(Math.ceil((closingDate - new Date()) / (1000 * 60 * 60 * 24)), 0);

  return (
    <Dialog open={isOpen} onClose={onClose} className="fixed z-50 inset-0 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 py-8">
        <Dialog.Panel className="bg-[#0f172a] max-w-3xl w-full p-8 rounded-xl shadow-xl border border-gray-700 text-white relative">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-300 leading-relaxed mb-4 whitespace-pre-line">{job.detailed_description}</p>
            <div className="text-sm text-gray-500 border border-gray-700 rounded px-4 py-2 inline-block">
              <strong className="text-white">Application Deadline:</strong> {closingDate.toLocaleDateString('en-US')} at 11:59pm PT
            </div>
          </div>
